...
class App {
  ...
  async init() {
    ...
    this.brushTool = new BrushTool(this.state, this.overlay);
  ...
  _bindEvents() {
    ...
    canvas.addEventListener('mousemove', (e) => {
      ...
      if (this.state.state === States.DRAWING_POLYGON) {
        this.brushTool.onMouseMove(pos);
    ...
    canvas.addEventListener('mousedown', (e) => {
      ...
      if (this.state.state === States.DRAWING_POLYGON) {
        this.brushTool.onMouseDown(pos);
      }
    });
    ...
    canvas.addEventListener('mouseup', (e) => {
      if (e.button !== 0) return;
      const pos = this.overlay.screenToCanvas(e.clientX, e.clientY);
      console.log('[Canvas] mouseup at display:', e.clientX, e.clientY, '-> canvas:', Math.round(pos.x), Math.round(pos.y), 'state:', this.state.state);

      if (this.state.state === States.DRAWING_POLYGON) {
        this.brushTool.onMouseUp(pos);
      } else if (this.state.state === States.DRAWING_LINE) {
    ...
    canvas.addEventListener('mouseleave', () => {
      this._cursorPos = null;
      if (this.brushTool) this.brushTool.onMouseMove(null);
    ...
    document.addEventListener('keydown', (e) => {
      ...
      if (s === States.DRAWING_POLYGON) {
        if (e.key === 'Escape') {
          this.brushTool.onKeyDown(e);
        }
      } else if (s === States.DRAWING_LINE) {
        this.lineTool.onKeyDown(e);
      } else if (s === States.IDLE) {
        if ((e.key === 'b' || e.key === 'B') && !document.getElementById('btn-brush-tool').classList.contains('active')) {
          document.getElementById('btn-brush-tool').click();
        }
        if (e.key === ' ') {
          e.preventDefault();
    ...
    document.getElementById('btn-brush-tool').addEventListener('click', () => {
      console.log('[BrushBtn] clicked, current state:', this.state.state);
    ...
    document.getElementById('btn-move-option2').addEventListener('click', () => {
      window.location.href = '../option1/index.html';
    });
  }
  ...
  _bindStateListeners() {
    this.state.on('stateChange', (data) => {
      ...
      const brushBtn = document.getElementById('btn-brush-tool');
      const lineBtn = document.getElementById('btn-line-tool');

      if (data.next === States.IDLE) {
        // Auto-activate brush tool on IDLE
  ...
  _updateUI() {
    ...
    const showBrush = s === States.IDLE || s === States.DRAWING_POLYGON;
    const activeRoad = this.state.getActiveRoad();
    const hasLine = activeRoad && activeRoad.line;
    const showLine = s === States.POLYGON_COMPLETE && !hasLine;
    const showErase = s === States.EDITING;

    brushBtn.style.display = showBrush ? '' : 'none';
    brushBtn.classList.toggle('active', s === States.DRAWING_POLYGON);
  ...
  _getIcon(name) {
    const icons = {
      ...
      'brush-tool': '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18.37 2.63a2.12 2.12 0 0 1 3 3L14 13l-4 1 1-4Z"/><path d="M9 7c-3 0-6 2-6 6s3 5 6 5c1 0 3-.5 4-1.5"/><path d="M15.5 14.5c.5 1 1 2.5 1 4.5"/></svg>',
