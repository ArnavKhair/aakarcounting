...
export class AnnotationOverlay {
  constructor(canvas, video) {
    ...
    this._videoDisplayRect = { left: 0, top: 0, width: 0, height: 0 };
  ...
  _syncSize() {
    ...
    this._computeVideoDisplayRect();
    this._applyCanvasRect();
  }

  _computeVideoDisplayRect() {
  ...
  _applyCanvasRect() {
    const r = this._videoDisplayRect;
  ...
  screenToCanvas(screenX, screenY) {
    ...
    return {
      x: ((screenX - rect.left) / rect.width) * vw,
      y: ((screenY - rect.top) / rect.height) * vh,
    };
  }
  ...
  canvasToScreen(cx, cy) {
    ...
    return {
      x: rect.left + (cx / vw) * rect.width,
      y: rect.top + (cy / vh) * rect.height,
    };
  }
